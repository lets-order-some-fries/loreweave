...
export async function indexVault(
  ...
  for (const f of files) {
    ...
    if (prev && !opts.full && prev.mtimeMs === f.mtimeMs) {
      report.unchanged++;
      continue;
    }
    let raw: string;
    ...
    if (prev && !opts.full && prev.hash === sha1(raw)) {
      // touched but unchanged: record new mtime so next run short-circuits
      store.db.prepare(`UPDATE notes SET mtime_ms=? WHERE path=?`).run(f.mtimeMs, f.path);
      report.unchanged++;
      continue;
    }
    const note = parseNote(f.path, raw, f.mtimeMs);
